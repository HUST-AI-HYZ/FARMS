...
parser.add_argument('--pl-fitting',     type=str,        default='median', choices=['median', 'goodness-of-fit', 'fix-finger', 'xmin_peak'])
...
dir = os.path.join(args.ckpt_path, 'tb_stats')
if not os.path.exists(dir):
    os.makedirs(dir)

dir = os.path.join(args.ckpt_path, 'lr_stats')
if not os.path.exists(dir):
    os.makedirs(dir)
...
if args.use_tb:
    ...
    tb_param_group, _ = \
        tb_scheduler.build_optimizer_param_group(untuned_lr=args.lr, epoch=-1, initialize=True)
...
for epoch in range(start_epoch, start_epoch+args.num_epochs):
    ...
    if args.use_tb:
        ...
        tb_scheduler.step(optimizer, untuned_lr, epoch=epoch)
    else:
